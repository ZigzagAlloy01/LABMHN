...
def drawGraph(list, listTwo, m, b, sm, sb):
    ...
    fig.text(0.45, 0.24, f'qe = 0.0101 (mg g⁻¹)', fontsize = 24, color ='black', ha ='right', va ='bottom', alpha = 1)
    fig.text(0.45, 0.17, f'K₁ = 0.0059 (min⁻¹)', fontsize = 24, color ='black', ha ='right', va ='bottom', alpha = 1)
    fig.text(0.45, 0.13, f'R{chr(0x00B2)} = {coefficientOfDetermination(list, listTwo):.4f}',
        fontsize = 24, color ='black',
        ha ='right', va ='center',
        alpha = 1)
    #plt.grid(alpha =.6, linestyle ='--')
    plt.title(f'Cinética de Pseudo Primer Orden: Al₂O₃', fontsize = 24)
    plt.ylabel('ln(qe-qt)', fontsize = 24)
    plt.xlabel(f't (min)', fontsize = 24)
    plt.xlim((0), (max(x) + 20))
    plt.ylim(-6.4, (max(y)) + 0.3)
